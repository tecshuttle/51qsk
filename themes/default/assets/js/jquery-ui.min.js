 ...
 */
var Y=a.widget("ui.tooltip",{version:"1.11.1",options:{content:function(){var ae=a(this).attr("title")||"";return a("<a>").text(ae).html()},hide:true,items:"[title]:not([disabled])",position:{my:"left top+15",at:"left bottom",collision:"flipfit flip"},show:true,tooltipClass:null,track:false,close:null,open:null},_addDescribedBy:function(af,ag){var ae=(af.attr("aria-describedby")||"").split(/\s+/);ae.push(ag);af.data("ui-tooltip-id",ag).attr("aria-describedby",a.trim(ae.join(" ")))},_removeDescribedBy:function(af){var ag=af.data("ui-tooltip-id"),ae=(af.attr("aria-describedby")||"").split(/\s+/),ah=a.inArray(ag,ae);if(ah!==-1){ae.splice(ah,1)}af.removeData("ui-tooltip-id");ae=a.trim(ae.join(" "));if(ae){af.attr("aria-describedby",ae)}else{af.removeAttr("aria-describedby")}},_create:function(){this._on({mouseover:"open",focusin:"open"});this.tooltips={};this.parents={};if(this.options.disabled){this._disable()}this.liveRegion=a("<div>").attr({role:"log","aria-live":"assertive","aria-relevant":"additions"}).addClass("ui-helper-hidden-accessible").appendTo(this.document[0].body)},_setOption:function(ae,ag){var af=this;if(ae==="disabled"){this[ag?"_disable":"_enable"]();this.options[ae]=ag;return}this._super(ae,ag);if(ae==="content"){a.each(this.tooltips,function(ai,ah){af._updateContent(ah)})}},_disable:function(){var ae=this;a.each(this.tooltips,function(ah,af){var ag=a.Event("blur");ag.target=ag.currentTarget=af[0];ae.close(ag,true)});this.element.find(this.options.items).addBack().each(function(){var af=a(this);if(af.is("[title]")){af.data("ui-tooltip-title",af.attr("title")).removeAttr("title")}})},_enable:function(){this.element.find(this.options.items).addBack().each(function(){var ae=a(this);if(ae.data("ui-tooltip-title")){ae.attr("title",ae.data("ui-tooltip-title"))}})},open:function(ae){var ag=this,af=a(ae?ae.target:this.element).closest(this.options.items);if(!af.length||af.data("ui-tooltip-id")){return}if(af.attr("title")){af.data("ui-tooltip-title",af.attr("title"))}af.data("ui-tooltip-open",true);if(ae&&ae.type==="mouseover"){af.parents().each(function(){var ai=a(this),ah;if(ai.data("ui-tooltip-open")){ah=a.Event("blur");ah.target=ah.currentTarget=this;ag.close(ah,true)}if(ai.attr("title")){ai.uniqueId();ag.parents[this.id]={element:this,title:ai.attr("title")};ai.attr("title","")}})}this._updateContent(af,ae)},_updateContent:function(ai,ag){var ae,af=this.options.content,aj=this,ah=ag?ag.type:null;if(typeof af==="string"){return this._open(ag,ai,af)}ae=af.call(ai[0],function(ak){if(!ai.data("ui-tooltip-open")){return}aj._delay(function(){if(ag){ag.type=ah}this._open(ag,ai,ak)})});if(ae){this._open(ag,ai,ae)}},_open:function(ah,al,af){var am,ai,ag,ae,ak=a.extend({},this.options.position);if(!af){return}am=this._find(al);if(am.length){am.find(".ui-tooltip-content").html(af);return}if(al.is("[title]")){if(ah&&ah.type==="mouseover"){al.attr("title","")}else{al.removeAttr("title")}}am=this._tooltip(al);this._addDescribedBy(al,am.attr("id"));am.find(".ui-tooltip-content").html(af);this.liveRegion.children().hide();if(af.clone){ae=af.clone();ae.removeAttr("id").find("[id]").removeAttr("id")}else{ae=af}a("<div>").html(ae).appendTo(this.liveRegion);function aj(an){ak.of=an;if(am.is(":hidden")){return}am.position(ak)}if(this.options.track&&ah&&/^mouse/.test(ah.type)){this._on(this.document,{mousemove:aj});aj(ah)}else{am.position(a.extend({of:al},this.options.position))}this.hiding=false;this.closing=false;am.hide();this._show(am,this.options.show);if(this.options.show&&this.options.show.delay){ag=this.delayedShow=setInterval(function(){if(am.is(":visible")){aj(ak.of);clearInterval(ag)}},a.fx.interval)}this._trigger("open",ah,{tooltip:am});ai={keyup:function(an){if(an.keyCode===a.ui.keyCode.ESCAPE){var ao=a.Event(an);ao.currentTarget=al[0];this.close(ao,true)}}};if(al[0]!==this.element[0]){ai.remove=function(){this._removeTooltip(am)}}if(!ah||ah.type==="mouseover"){ai.mouseleave="close"}if(!ah||ah.type==="focusin"){ai.focusout="close"}this._on(true,al,ai)},close:function(ae){var ag=this,af=a(ae?ae.currentTarget:this.element),ah=this._find(af);if(this.closing){return}clearInterval(this.delayedShow);if(af.data("ui-tooltip-title")&&!af.attr("title")){af.attr("title",af.data("ui-tooltip-title"))}this._removeDescribedBy(af);this.hiding=true;ah.stop(true);this._hide(ah,this.options.hide,function(){ag._removeTooltip(a(this));this.hiding=false;this.closing=false});af.removeData("ui-tooltip-open");this._off(af,"mouseleave focusout keyup");if(af[0]!==this.element[0]){this._off(af,"remove")}this._off(this.document,"mousemove");if(ae&&ae.type==="mouseleave"){a.each(this.parents,function(ai,aj){a(aj.element).attr("title",aj.title);delete ag.parents[ai]})}this.closing=true;this._trigger("close",ae,{tooltip:ah});if(!this.hiding){this.closing=false}},_tooltip:function(ae){var ag=a("<div>").attr("role","tooltip").addClass("ui-tooltip ui-widget ui-corner-all ui-widget-content "+(this.options.tooltipClass||"")),af=ag.uniqueId().attr("id");a("<div>").addClass("ui-tooltip-content").appendTo(ag);ag.appendTo(this.document[0].body);this.tooltips[af]=ae;return ag},_find:function(af){var ae=af.data("ui-tooltip-id");return ae?a("#"+ae):a()},_removeTooltip:function(ae){ae.remove();delete this.tooltips[ae.attr("id")]},_destroy:function(){var ae=this;a.each(this.tooltips,function(ah,af){var ag=a.Event("blur");ag.target=ag.currentTarget=af[0];ae.close(ag,true);a("#"+ah).remove();if(af.data("ui-tooltip-title")){if(!af.attr("title")){af.attr("title",af.data("ui-tooltip-title"))}af.removeData("ui-tooltip-title")}});this.liveRegion.remove()}})}));