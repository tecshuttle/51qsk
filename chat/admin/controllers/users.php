...
class c_users extends Admin{
	...
	private function DeleteUser($aid){
		if(!$aid) return;
		APP::$DB->exe("DELETE FROM " . TABLE_PREFIX . "admin WHERE aid = '$aid'");

		//删除常用短语
		APP::$DB->exe("DELETE FROM " . TABLE_PREFIX . "phrase WHERE aid = '$aid'");
	...
	public function index(){
		...
		while($user = APP::$DB->fetch($getusers)){
			...
			Iif($user['aid'] != $this->admin['aid'], '<input type="checkbox" name="deleteaids[]" value="' . $user['aid'] . '">')));
		}
